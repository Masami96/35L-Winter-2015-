...
//the following variables were part of main(), but were used in the thread work so have to make them global
int nthreads;
scene_t scene;
double scaled_color[width][height][3];

void* thread_work(void* arg) //arg ptr indicates which thread is working
{
...
main( int argc, char **argv )
{
    ...
    //being creating the appropriate number of threads, have them do the work on creation
    //make all memory allocation failures return -1
    ...
    for (int i = 0; i < nthreads; i++)
      {
	int pos_error = pthread_create(tids+i, NULL, thread_work, &i);
	if (pos_error)
	  return 1;
      }
